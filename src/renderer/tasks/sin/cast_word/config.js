class CaSTWordConfig {
    constructor() {
        this.configName = 'cfg_cast_word_task.json';
        this.defaultConfig = {
            task: 'cast-word',
            parameters: {
                audio: {
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('CaST Word config loaded:', this.currentConfig);
        } catch (error) {
    ...
    async saveConfig() {
        ...
        console.log('CaST Word config saved to:', configPath);
    }

    generateConfigHTML() {
        return `
            <div class="modal-header">
                <h2 class="modal-title">CaST Word Task Configuration</h2>
            ...
            <div class="modal-body">
                ...
                <div class="config-tab-content active" id="audio-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="cast-word-audio-volume">Volume</label>
                            <div class="slider-control">
                                <div class="slider-value">
                                    <span>Volume Level</span>
                                    <span class="slider-value-display" id="cast-word-volume-value">${Math.round(this.currentConfig.parameters.audio.volume * 100)}%</span>
                                </div>
                                <input type="range" id="cast-word-audio-volume" name="volume" 
                ...
                <div class="config-tab-content" id="data-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="cast-word-crash-recovery">Enable Crash Recovery Logs</label>
                            <div class="toggle-switch">
                                <input type="checkbox" id="cast-word-crash-recovery" name="crash_recovery" 
    ...
    bindConfigEvents() {
        ...
        const volumeSlider = document.getElementById('cast-word-audio-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const value = parseFloat(e.target.value);
                document.getElementById('cast-word-volume-value').textContent = Math.round(value * 100) + '%';
                this.currentConfig.parameters.audio.volume = value;
            });
        }

        const crashRecoveryCheckbox = document.getElementById('cast-word-crash-recovery');
    ...
    async loadExistingConfiguration() {
        try {
            ...
            console.log('No existing CaST Word configuration found, using defaults');
        }
    }
    ...
    updateUIFromConfig() {
        ...
        if (volumeSlider) {
            ...
            document.getElementById('cast-word-volume-value').textContent = 
                Math.round(this.currentConfig.parameters.audio.volume * 100) + '%';
        }

        const crashRecovery = document.getElementById('cast-word-crash-recovery');
        if (crashRecovery) {
...
window.CaSTWordConfig = CaSTWordConfig;
