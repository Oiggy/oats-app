class HINTConfig {
    constructor() {
        this.configName = 'cfg_hint_task.json';
        this.defaultConfig = {
            task: 'hint',
            parameters: {
                audio: {
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('HINT config loaded:', this.currentConfig);
        } catch (error) {
    ...
    async saveConfig() {
        ...
        console.log('HINT config saved to:', configPath);
    }

    generateConfigHTML() {
        return `
            <div class="modal-header">
                <h2 class="modal-title">HINT Task Configuration</h2>
            ...
            <div class="modal-body">
                ...
                <div class="config-tab-content active" id="audio-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="hint-audio-volume">Volume</label>
                            <div class="slider-control">
                                <div class="slider-value">
                                    <span>Volume Level</span>
                                    <span class="slider-value-display" id="hint-volume-value">${Math.round(this.currentConfig.parameters.audio.volume * 100)}%</span>
                                </div>
                                <input type="range" id="hint-audio-volume" name="volume" 
                ...
                <div class="config-tab-content" id="data-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="hint-crash-recovery">Enable Crash Recovery Logs</label>
                            <div class="toggle-switch">
                                <input type="checkbox" id="hint-crash-recovery" name="crash_recovery" 
    ...
    bindConfigEvents() {
        ...
        const volumeSlider = document.getElementById('hint-audio-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const value = parseFloat(e.target.value);
                document.getElementById('hint-volume-value').textContent = Math.round(value * 100) + '%';
                this.currentConfig.parameters.audio.volume = value;
            });
        }

        const crashRecoveryCheckbox = document.getElementById('hint-crash-recovery');
    ...
    async loadExistingConfiguration() {
        try {
            ...
            console.log('No existing HINT configuration found, using defaults');
        }
    }
    ...
    updateUIFromConfig() {
        ...
        if (volumeSlider) {
            ...
            document.getElementById('hint-volume-value').textContent = 
                Math.round(this.currentConfig.parameters.audio.volume * 100) + '%';
        }

        const crashRecovery = document.getElementById('hint-crash-recovery');
        if (crashRecovery) {
