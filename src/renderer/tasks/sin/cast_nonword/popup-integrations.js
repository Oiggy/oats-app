class CaSTNonwordTask {
    ...
    async loadConfiguration() {
        ...
        const configPath = path.join(baseDir, 'cfg_cast_nonword_task.json');
    ...
    createTaskModal() {
        ...
        this.modalContent.className = 'task-modal-content cast-nonword-modal';
...
refreshPlayerUI() {
    ...
    cleanup() {
        console.log('CaST Non-word task cleanup completed');
        window.castNonwordTaskInstance = null;
    }
}

// Global function to load and start the CaST Non-word task
async function loadCaSTNonwordTask(participantId) {
    try {
        console.log('Loading CaST Non-word task for participant:', participantId);
        
        window.castNonwordTaskInstance = new CaSTNonwordTask(participantId);
        await window.castNonwordTaskInstance.init();
...
window.loadCaSTNonwordTask = loadCaSTNonwordTask;