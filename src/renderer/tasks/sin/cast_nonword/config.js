class CaSTNonwordConfig {
    constructor() {
        this.configName = 'cfg_cast_nonword_task.json';
        this.defaultConfig = {
            task: 'cast-nonword',
            parameters: {
                audio: {
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('CaST Non-word config loaded:', this.currentConfig);
        } catch (error) {
    ...
    async saveConfig() {
        ...
        console.log('CaST Non-word config saved to:', configPath);
    }

    generateConfigHTML() {
        return `
            <div class="modal-header">
                <h2 class="modal-title">CaST Non-word Task Configuration</h2>
            ...
            <div class="modal-body">
                ...
                <div class="config-tab-content active" id="audio-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="cast-nonword-audio-volume">Volume</label>
                            <div class="slider-control">
                                <div class="slider-value">
                                    <span>Volume Level</span>
                                    <span class="slider-value-display" id="cast-nonword-volume-value">${Math.round(this.currentConfig.parameters.audio.volume * 100)}%</span>
                                </div>
                                <input type="range" id="cast-nonword-audio-volume" name="volume" 
                ...
                <div class="config-tab-content" id="data-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="cast-nonword-crash-recovery">Enable Crash Recovery Logs</label>
                            <div class="toggle-switch">
                                <input type="checkbox" id="cast-nonword-crash-recovery" name="crash_recovery" 
    ...
    bindConfigEvents() {
        ...
        const volumeSlider = document.getElementById('cast-nonword-audio-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const value = parseFloat(e.target.value);
                document.getElementById('cast-nonword-volume-value').textContent = Math.round(value * 100) + '%';
                this.currentConfig.parameters.audio.volume = value;
            });
        }

        const crashRecoveryCheckbox = document.getElementById('cast-nonword-crash-recovery');
    ...
    async loadExistingConfiguration() {
        try {
            ...
            console.log('No existing CaST Non-word configuration found, using defaults');
        }
    }
    ...
    updateUIFromConfig() {
        ...
        if (volumeSlider) {
            ...
            document.getElementById('cast-nonword-volume-value').textContent = 
                Math.round(this.currentConfig.parameters.audio.volume * 100) + '%';
        }

        const crashRecovery = document.getElementById('cast-nonword-crash-recovery');
        if (crashRecovery) {
...
window.CaSTNonwordConfig = CaSTNonwordConfig;
