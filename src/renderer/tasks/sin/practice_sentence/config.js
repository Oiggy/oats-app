class PracticeSentenceConfig {
    constructor() {
        this.configName = 'cfg_practice_sentence_task.json';
        this.defaultConfig = {
            task: 'practice-sentence',
            parameters: {
                audio: {
                    volume: 1.0
                },
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('Practice Sentence config loaded:', this.currentConfig);
        } catch (error) {
    ...
    async saveConfig() {
        ...
        console.log('Practice Sentence config saved to:', configPath);
    }

    generateConfigHTML() {
        return `
            <div class="modal-header">
                <h2 class="modal-title">Practice Sentence Task Configuration</h2>
            ...
            <div class="modal-body">
                ...
                <div class="config-tab-content active" id="audio-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="ps-audio-volume">Volume</label>
                            <div class="slider-control">
                                <div class="slider-value">
                                    <span>Volume Level</span>
                                    <span class="slider-value-display" id="ps-volume-value">${Math.round(this.currentConfig.parameters.audio.volume * 100)}%</span>
                                </div>
                                <input type="range" id="ps-audio-volume" name="volume" 
                ...
                <div class="config-tab-content" id="data-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="ps-crash-recovery">Enable Crash Recovery Logs</label>
                            <div class="toggle-switch">
                                <input type="checkbox" id="ps-crash-recovery" name="crash_recovery" 
    ...
    bindConfigEvents() {
        ...
        // Volume slider
        const volumeSlider = document.getElementById('ps-audio-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const value = parseFloat(e.target.value);
                document.getElementById('ps-volume-value').textContent = Math.round(value * 100) + '%';
                this.currentConfig.parameters.audio.volume = value;
            });
        }

        // Crash recovery toggle
        const crashRecoveryCheckbox = document.getElementById('ps-crash-recovery');
    ...
    async loadExistingConfiguration() {
        try {
            ...
            console.log('No existing Practice Sentence configuration found, using defaults');
        }
    }
    ...
    updateUIFromConfig() {
        ...
        if (volumeSlider) {
            ...
            document.getElementById('ps-volume-value').textContent = 
                Math.round(this.currentConfig.parameters.audio.volume * 100) + '%';
        }

        // Update crash recovery
        const crashRecovery = document.getElementById('ps-crash-recovery');
        if (crashRecovery) {
...
window.PracticeSentenceConfig = PracticeSentenceConfig;
