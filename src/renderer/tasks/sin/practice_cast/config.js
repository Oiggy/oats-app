class PracticeCastConfig {
    constructor() {
        this.configName = 'cfg_practice_cast_task.json';
        this.defaultConfig = {
            task: 'practice-cast',
            parameters: {
                audio: {
    ...
    async loadConfig() {
        ...
        try {
            ...
            console.log('Practice CaST config loaded:', this.currentConfig);
        } catch (error) {
    ...
    async saveConfig() {
        ...
        console.log('Practice CaST config saved to:', configPath);
    }

    generateConfigHTML() {
        return `
            <div class="modal-header">
                <h2 class="modal-title">Practice CaST Task Configuration</h2>
            ...
            <div class="modal-body">
                ...
                <div class="config-tab-content active" id="audio-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="pc-audio-volume">Volume</label>
                ...
                <div class="config-tab-content" id="data-tab">
                    <div class="config-card">
                        ...
                        <div class="config-group">
                            <label for="pc-crash-recovery">Enable Crash Recovery Logs</label>
                            <div class="toggle-switch">
                                <input type="checkbox" id="pc-crash-recovery" name="crash_recovery" 
    ...
    bindConfigEvents() {
        ...
        // Volume slider
        const volumeSlider = document.getElementById('pc-audio-volume');
        if (volumeSlider) {
            volumeSlider.addEventListener('input', (e) => {
                const value = parseFloat(e.target.value);
                document.getElementById('pc-volume-value').textContent = Math.round(value * 100) + '%';
                this.currentConfig.parameters.audio.volume = value;
            });
        }

        // Crash recovery toggle
        const crashRecoveryCheckbox = document.getElementById('pc-crash-recovery');
    ...
    async loadExistingConfiguration() {
        try {
            ...
            console.log('No existing Practice CaST configuration found, using defaults');
        }
    }
    ...
    updateUIFromConfig() {
        ...
        if (volumeSlider) {
            ...
            document.getElementById('pc-volume-value').textContent = 
                Math.round(this.currentConfig.parameters.audio.volume * 100) + '%';
        }

        // Update crash recovery
        const crashRecovery = document.getElementById('pc-crash-recovery');
        if (crashRecovery) {
...
window.PracticeCastConfig = PracticeCastConfig;
